...
def cosine_similarity(A, B):
    ...
    norma = np.linalg.norm(A)             # norm of A
    normb = np.linalg.norm(B)             # norm of B
    cos = dot / (norma * normb)           # cosine similarity
    ### END CODE HERE ###
...
def euclidean(A, B):
    ...
    """
    ### START CODE HERE ###
    d = np.linalg.norm(A - B)   # directly compute using numpy norm
    ### END CODE HERE ###
...
def get_country(city1, country1, city2, word_embeddings, cosine_similarity=cosine_similarity):
    ...
    Input:
        ...
        cosine_similarity: function to compute cosine similarity
    Output:
        country: a tuple (best matching country, similarity score)
    """
    ### START CODE HERE ###

    # store the words that must be excluded
    ...
    # vector analogy: country2 ≈ country1 - city1 + city2
    ...
    # loop through all words in embeddings dictionary
    for word, word_emb in word_embeddings.items():

        if word not in group:  # exclude input words
...
def get_accuracy(word_embeddings, data, get_country=get_country):
    ...
    for i, row in data.iterrows():
        # unpack city-country pairs
        city1 = row['city1']
        country1 = row['country1']
        city2 = row['city2']
        country2 = row['country2']
        
        # predict using analogy function
...
def compute_pca(X, n_components=2):
    ...
    Output:
        X_reduced: data transformed in n_components dims
    """
    ### START CODE HERE ###
    ...
    idx_sorted = np.argsort(eigen_vals)        # ascending order
    idx_sorted_decreasing = idx_sorted[::-1]   # reverse → descending
